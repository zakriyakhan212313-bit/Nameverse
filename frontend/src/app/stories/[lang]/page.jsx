...
// --- Data Fetch ---
async function getStoryData(lang) {
  try {
    const apiUrl = `${APIBASE}/stories/${lang}`;
...
export default async function StoryPage({ params }) {
  ...
  const apiData = await getStoryData(lang);
  ...
  const transformedData = {
    ...
    read_time_minutes: apiData.read_time_minutes || apiData.readtimeminutes || 0,
    difficulty_level: apiData.difficulty_level || apiData.difficultylevel || "easy",
    tags: apiData.tags || [],
    category: apiData.category || "General",
    sub_category: apiData.sub_category || apiData.subcategory || "",
...
export async function generateMetadata({ params }) {
  ...
  const storyData = await getStoryData(lang);
  if (!storyData)
    return {
      title: 'Story Not Found',
      description: 'The requested story could not be found.',
